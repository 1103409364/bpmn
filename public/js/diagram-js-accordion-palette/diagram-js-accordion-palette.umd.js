(function (global, factory) {
    typeof exports === 'object' && typeof module !== 'undefined' ? module.exports = factory(require('min-dash'), require('min-dom')) :
    typeof define === 'function' && define.amd ? define(['min-dash', 'min-dom'], factory) :
    (global = typeof globalThis !== 'undefined' ? globalThis : global || self, global["diagram-js-accordion-palette"] = factory(global.MinDash, global.MinDom));
})(this, (function (minDash, minDom) { 'use strict';
    ...
    ***************************************************************************** */
    ...
    var AccordionPalette = /** @class */ (function () {
        ...
        AccordionPalette.prototype._init = function () {
            ...
            var container = this._container = minDom.domify(AccordionPalette.HTML_MARKUP);
            parentContainer.appendChild(container);
            minDom.classes(parentContainer).add(PALETTE_PREFIX + PALETTE_SHOWN_CLS);
            minDom.event.bind(container, 'mousedown', function (event) {
                event.stopPropagation();
            });
            minDom.delegate.bind(container, ENTRY_SELECTOR, 'click', function (event) {
                self.trigger('click', event);
            });
            minDom.delegate.bind(container, ENTRY_SELECTOR, 'dragstart', function (event) {
                self.trigger('dragstart', event);
            });
            minDom.event.bind(container, 'mousewheel', function (event) {
                event.stopPropagation();
                event.stopImmediatePropagation();
            });
            minDom.event.bind(container, 'wheel', function (event) {
                event.stopPropagation();
                event.stopImmediatePropagation();
            });
        ...
        AccordionPalette.prototype._update = function () {
            ...
            var entriesContainer = minDom.query('.djs-palette-entries', this._container);
            ...
            minDash.forEach(entries, function (entry, id) {
                if (entry.separator) {
                    return;
                }
                ...
                var detailsContainer = minDom.query("[data-group-details=".concat(groupName, "]"), entriesContainer);
                if (!detailsContainer) {
                    detailsContainer = minDom.domify("<details class=\"djs-accordion-group\" name=\"".concat(grouping, "\"></details>"));
                    minDom.attr(detailsContainer, 'data-group-details', groupName);
                    entriesContainer.appendChild(detailsContainer);
                    var summaryContainer = minDom.domify("<summary>".concat(translate(entry.group || 'default'), "</summary>"));
                    detailsContainer.appendChild(summaryContainer);
                }
                if (defaultOpenMap[entry.group]) {
                    minDom.attr(detailsContainer, 'open', 'true');
                }
                // 2. 生成 details 标签的内容主体
                var groupContainer = minDom.query("[data-group=".concat(groupName, "]"), detailsContainer);
                if (!groupContainer) {
                    groupContainer = minDom.domify("<div class=\"djs-palette-group\"></div>");
                    minDom.attr(groupContainer, 'data-group', groupName);
                ...
                var entryEl = minDom.domify(html);
                var entryItemEl;
                if (showName) {
                    var control = minDom.domify("<div class=\"djs-entry-item djs-entry-with-name\"></div>");
                ...
                minDom.attr(entryEl, 'data-action', id);
                if (!minDom.classes(entryEl).has('entry')) {
                    addClasses(entryEl, 'entry');
                }
                // 4. 设置其他内容
                if (entry.title) {
                    minDom.attr(entryItemEl, 'title', entry.title);
                    if (showName) {
                        var name_1 = minDom.domify("<div class=\"djs-entry-title\">".concat(entry.title, "</div>"));
                ...
                if (entry.imageUrl) {
                    var image = minDom.domify('<img class="djs-entry-img">');
                    minDom.attr(image, 'src', entry.imageUrl);
        ...
        AccordionPalette.prototype.trigger = function (action, event, autoActivate) {
            ...
            entry = minDom.attr(button, 'data-action');
        ...
        AccordionPalette.prototype._toggleVisible = function (state) {
            ...
            var cls = minDom.classes(container);
            var parentCls = minDom.classes(parent);
        ...
        AccordionPalette.prototype.updateToolHighlight = function (name) {
            ...
            if (!this._toolsContainer) {
                entriesContainer = minDom.query('.djs-palette-entries', this._container);
                this._toolsContainer = minDom.query('[data-group=tools]', entriesContainer);
            }
            toolsContainer = this._toolsContainer;
            minDash.forEach(toolsContainer.children, function (tool) {
                var actionName = tool.getAttribute('data-action');
                if (!actionName) {
                    return;
                }
                var toolClasses = minDom.classes(tool);
        ...
        AccordionPalette.prototype.isOpen = function () {
            return minDom.classes(this._container).has(PALETTE_OPEN_CLS);
        };
    ...
    function addClasses(element, classNames) {
        var classes = minDom.classes(element);
        var actualClassNames = minDash.isArray(classNames) ? classNames : classNames.split(/\s+/g);
        actualClassNames.forEach(function (cls) { return classes.add(cls); });
    }
